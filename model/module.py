...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def _update_cos_sin_cache(self, x, seq_len):
        if seq_len != self.seq_len_cached:
            ...
            self.cos_cached, self.sin_cached = self._compute_cos_sin(x, seq_len)
    ...
    def forward(self, q, k):
        ...
        k_len = k.shape[-2]
        if q_len == k_len:
            cos, sin = self._update_cos_sin_cache(q, q_len)
            return (
                (q * cos) + (self._rotate_half(q) * sin),
                (k * cos) + (self._rotate_half(k) * sin),
            )
        else:
            # Cross-attention: q and k have different lengths
            q_cos, q_sin = self._compute_cos_sin(q, q_len)
            k_cos, k_sin = self._compute_cos_sin(k, k_len)
            return (
